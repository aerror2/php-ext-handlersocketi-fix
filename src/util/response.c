...
hs_response_recv(php_stream *stream, char *recv, size_t size TSRMLS_DC)
{
    ...
    do
    {
          ...
          if(ret==0 && hs_response_select(stream,5)==-2)
          {
            break;
          }
    }while(ret==0 && !php_stream_eof(stream));
...
#define HS_RESPONSE_SELECT_FAILED "hs_response_select failed"
void
                  ...
                  zval *error, int modify TSRMLS_DC)
{
    ...
    if ( hs_response_select(stream, timeout TSRMLS_CC) < 0) {
           if (error) {
                ZVAL_STRINGL(error, HS_RESPONSE_SELECT_FAILED, sizeof(HS_RESPONSE_SELECT_FAILED)-1, 1);
            }
    ...
    if ( len <= 0) {
        ...
        if (error) {
                char szError[50];
                sprintf(szError,"hs_response_recv return %ld",len);
                ZVAL_STRINGL(error, szError, strlen(szError), 1);
        }
        ...
        if (recv[i] != HS_CODE_EOL) {
            ...
            do {
                for (j = i; j < len; j++) {
                 ...
                 //to break again from the for loop before
                if (j<len &&  recv[j] == HS_CODE_EOL) {
                    break;
                }

                i = 0;
                len = hs_response_recv(
                          stream, recv, block_size TSRMLS_CC);
                if (error) {
                    char szError[50];
                    sprintf(szError,"hs_response_recv return %ld",len);
                    ZVAL_STRINGL(error, szError, strlen(szError), 1);
            ...
            if (flag > 1) {
                ...
                if (len <= 0) {
                    if (error) {
                        char szError[50];
                        sprintf(szError,"hs_response_recv return %ld",len);
                        add_next_index_stringl(error, szError, strlen(szError), 1);
                    }
