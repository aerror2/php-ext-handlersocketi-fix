...
hs_request_send(php_stream *stream, smart_str *request TSRMLS_DC)
{
    ...
    if (php_stream_set_option(stream, PHP_STREAM_OPTION_BLOCKING,
                              1, NULL) == -1) {
        zend_error(E_WARNING,
                   "HandlerSocketi: Un set blocking mode on a stream");
    }
     
    ret= php_stream_write(stream, request->c, request->len);
