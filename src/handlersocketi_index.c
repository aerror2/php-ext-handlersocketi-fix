                     ...
                     zval *fields, zval *options TSRMLS_DC)
{
    ...
    if (db_len == 0) {
        HS_EXCEPTION("hs_index_object_init(): invalid database %s", db);
        return;
    }

    if (table_len == 0) {
        HS_EXCEPTION("hs_index_object_init(): invalid table %s", table);
        return;
    }

    fields_str = hs_zval_to_comma_string(fields);
    if (!fields_str) {
        HS_EXCEPTION("hs_index_object_init(): invalid fields");
        return;
    }
    ...
    if (!index) {
        ...
        HS_EXCEPTION("hs_index_object_init(): invalid index");
        return;
    }
    ...
    *hsi->name = *index;
    ...
    if (hs_request_send(stream, &request TSRMLS_CC) < 0) {
        ...
        HS_EXCEPTION("hs_index_object_init(): invalid request send");
        return;
    }
    ...
    if (Z_TYPE_P(retval) == IS_BOOL && Z_LVAL_P(retval) == 0) {
        HS_EXCEPTION("hs_index_object_init(): unable to open index: %ld %p: %s",
                     id,
                     hsi->link,
                     hsi->error == NULL ?
...
ZEND_METHOD(HandlerSocketi_Index, find)
{
    ...
    if (options != NULL && Z_TYPE_P(options) == IS_ARRAY) {
        /* limit */
        limit = hs_get_options_long(HASH_OF(options), "limit",
                                    limit TSRMLS_CC);
        /* offset */
        offset = hs_get_options_long(HASH_OF(options), "offset",
                                     offset TSRMLS_CC);
        /* safe */
    ...
    if (fnum < hsi->field_num) {
        ...
        */
        zend_error(E_WARNING,
                   "HandlerSocketi_Index::insert(): invalid field count %ld", fnum);
    }
...
ZEND_METHOD(HandlerSocketi_Index, update)
{
    ...
    if (fnum > hsi->field_num) {
        ...
        */
        zend_error(E_WARNING,
                   "HandlerSocketi_Index::update(): invalid field count %ld : %ld", fnum,hsi->field_num);
    }
