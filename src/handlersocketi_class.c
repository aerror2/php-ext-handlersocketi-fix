...
ZEND_BEGIN_ARG_INFO_EX(arginfo_hs_method_close, 0, 0, 0)
ZEND_END_ARG_INFO()
...
ZEND_METHOD(HandlerSocketi, hs_close);
...
const zend_function_entry hs_methods[] = {
    ...
    ZEND_MALIAS(HandlerSocketi, close, hs_close,
                arginfo_hs_method_close, ZEND_ACC_PUBLIC)
...
ZEND_METHOD(HandlerSocketi, hs_close)
{
    hs_obj_t *hs = (hs_obj_t *)zend_object_store_get_object(getThis() TSRMLS_CC);
    HS_CHECK_OBJECT(hs, HandlerSocketi);
    if(hs->stream)
    {
        if(hs->persistent)
             php_stream_pclose(hs->stream);
        else
